...
from ox4rl.vis.post_evaluation.plotting import plot_confusion_matrix
from ox4rl.utils.bbox_matching import match_bounding_boxes
...
def eval_model_and_classifier(cfg):
    ...
    eval_folder = osp.join(cfg.evaldir, cfg.exp_name)
    
    # Define the latent dataset path and check if it already exists
    latent_base_path = osp.join(cfg.dataset_roots.ATARI, cfg.gamelist[0], "latents", dataset_mode)
    if not os.path.exists(latent_base_path) or not os.listdir(latent_base_path):
    ...
    clf, centroid_labels = load_classifier(folder_path=classifier_folder, clf_name="nn", data_subset_mode=data_subset_mode)
    ...
    conf_matrix = confusion_matrix_visualization(actual_labels_combined, predicted_labels_combined, label_list, game, eval_folder)
    ...
    df.to_csv(f"{eval_folder}/eval_model_and_classifier.csv")
...
def confusion_matrix_visualization(actual_labels_combined, predicted_labels_combined, label_list: List[Union[str, int]], game: str, base_path: str):
    ...
    plot_confusion_matrix(cm,
                          ...
                          path = osp.join(base_path, f"confusion_matrix_detection_{game}.png"),
