...
def main(cfg, dataset_mode):
    """
    Main function to create motion masks for a given dataset configuration and mode.
    Args:
        cfg (dict): Configuration dictionary containing dataset and training parameters.
        dataset_mode (str): Mode of the dataset (e.g., 'train', 'test').
    Returns:
        None
    This function performs the following steps:
    1. Initializes the dataset and dataloader.
    2. Checks if the motion path exists, and creates it if it does not.
    3. Iterates over the dataloader to process each batch of data.
    4. Reduces the motion masks using block reduction.
    5. Segments the reduced masks into individual masks.
    6. Saves each motion mask as a separate .pt file in the specified motion path.
    """
    # dataset = Atari_Z_What(cfg, dataset_mode, return_keys=["imgs", "motion"])
    dataset = Atari_Z_What(cfg, dataset_mode, return_keys=["imgs", "slot_latents", "slot_latents_labels"])

    train_dataloader = get_dataloader(cfg, dataset_mode, dataset)
    motion_path = dataset.motion_path  # Get correct motion path
    # if not os.path.exists(motion_path):
    #     print(f"⚠️ Motion folder not found. Creating: {motion_path}")
    #     os.makedirs(motion_path, exist_ok=True)
    for (index, data_dict) in tqdm(enumerate(train_dataloader)):
        print(f"Processing index {index}...")  # Debug print
