...
BASE_OUTPUT_PATH = os.path.join("..", "output") # defined relative to ox4rl/ox4rl folder from which the script is run
SHARED_OUTPUT_PATH = os.path.join(BASE_OUTPUT_PATH, "shared_eval_figures")
...
def plot_recall_per_object_type(final_dataframe_per_game):
    ...
    for game, df in final_dataframe_per_game.items():
        ...
        # save the figure
        game_path = os.path.join(BASE_OUTPUT_PATH, "checkpoints", game)
        plt.savefig(os.path.join(game_path, f"recall_per_object_type_{game}.png"))

def draw_precision_recall_curve(recalls, precisions, thresholds, filename, game):
    fig, ax = plt.subplots()
    ...
    for i, threshold in enumerate(thresholds):
        ...
        if i == 0 or recalls.iloc[i] != recalls.iloc[i-1] or precisions.iloc[i] != precisions.iloc[i-1]:
    ...
    game_path = os.path.join(BASE_OUTPUT_PATH, "checkpoints", game)
    plt.savefig(os.path.join(game_path, filename))
...
def plot_ap(final_dataframe_per_game):
    ...
    plt.savefig(os.path.join(SHARED_OUTPUT_PATH, "aps.png"))
...
def bar_plot(dataframe_per_game, baseline_results = None, metric="relevant_f_score", title="Localization", ylabel="F-score (%)"):
    ...
    os.makedirs(SHARED_OUTPUT_PATH, exist_ok=True)
    plt.savefig(os.path.join(SHARED_OUTPUT_PATH, f"bar_plot_{metric}{baseline_str}.png"))
