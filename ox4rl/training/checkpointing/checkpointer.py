from ox4rl.utils.niceprint import makedirs, pprint as print
...
class Checkpointer:
    ...
    def save_last(self, model, optimizer_fg, optimizer_bg, epoch, global_step):
        """Saves a checkpoint and ensures numbering continues correctly."""
    ...
    def load_last(self, model, optimizer_fg=None, optimizer_bg=None, device=None):
        """
        ...
        with open(self.listfile, 'rb') as f:
            model_list = pickle.load(f)
            if len(model_list) == 0:
                raise FileNotFoundError('Last checkpoint found.')
            else:
                path = model_list[-1]
    ...
    def save_best(self, metric_name, value, checkpoint, min_is_better):
        """
        Save the best checkpoint based on a given metric.
        """
