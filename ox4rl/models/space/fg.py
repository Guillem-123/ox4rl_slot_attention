...
from .space_utils import NumericalRelaxedBernoulli, kl_divergence_bern_bern, get_boundary_kernel_new, get_boundary_kernel
from .space_utils import spatial_transform, linear_annealing, inverse_spatial_transform
...
class SpaceFg(nn.Module):
    def __init__(self):
        ...
        self.fg_sigma = space_cfg.fg_sigma
        # I register many things as buffer but this is not really necessary.
        # Temperature for gumbel-softmax
        self.register_buffer('tau', torch.tensor(space_cfg.tau_start_value))

        # Priors
        # Some are buffer and some are not. This is stupid. But for compatibility I have to do so.
        self.register_buffer('prior_z_pres_prob', torch.tensor(space_cfg.z_pres_start_value))
        ...
        self.prior_scale_mean_new = torch.tensor(space_cfg.z_scale_mean_start_value)
        self.prior_scale_std_new = torch.tensor(space_cfg.z_scale_std_value)
        self.prior_shift_mean_new = torch.tensor(0.)
        self.prior_shift_std_new = torch.tensor(1.)
        # self.register_buffer('prior_scale_mean_new', torch.tensor(space_cfg.z_scale_mean_start_value))
        # self.register_buffer('prior_scale_std_new', torch.tensor(space_cfg.z_scale_std_value))
        ...
        self.register_buffer('prior_scale_mean',
                             torch.tensor([space_cfg.z_scale_mean_start_value] * 2).view((space_cfg.z_where_scale_dim), 1, 1))
        self.register_buffer('prior_scale_std',
                             torch.tensor([space_cfg.z_scale_std_value] * 2).view((space_cfg.z_where_scale_dim), 1, 1))
        self.register_buffer('prior_shift_mean',
                             torch.tensor([0., 0.]).view((space_cfg.z_where_shift_dim), 1, 1))
        self.register_buffer('prior_shift_std',
                             torch.tensor([1., 1.]).view((space_cfg.z_where_shift_dim), 1, 1))
    ...
    def anneal(self, global_step):
        ...
        self.prior_z_pres_prob = linear_annealing(self.prior_z_pres_prob.device, global_step,
                                                  space_cfg.z_pres_start_step, space_cfg.z_pres_end_step,
                                                  space_cfg.z_pres_start_value, space_cfg.z_pres_end_value)
        self.prior_scale_mean_new = linear_annealing(self.prior_z_pres_prob.device, global_step,
                                                     space_cfg.z_scale_mean_start_step, space_cfg.z_scale_mean_end_step,
                                                     space_cfg.z_scale_mean_start_value, space_cfg.z_scale_mean_end_value)
        self.tau = linear_annealing(self.tau.device, global_step,
                                    space_cfg.tau_start_step, space_cfg.tau_end_step,
                                    space_cfg.tau_start_value, space_cfg.tau_end_value)
    ...
    def forward(self, x, global_step):
        ...
        x_repeat = torch.repeat_interleave(x, space_cfg.G ** 2, dim=0)
        # (B*G*G, 3, H, W), where G is the grid size
        # Extract glimpse
        x_att = spatial_transform(x_repeat, z_where.view(B * space_cfg.G ** 2, 4),
                                  (B * space_cfg.G ** 2, 3, space_cfg.glimpse_size, space_cfg.glimpse_size))
        ...
        importance_map = alpha_att_hat * 100.0 * torch.sigmoid(-z_depth.view(B * space_cfg.G ** 2, 1, 1, 1))
        # (B*G*G, 1, H, W). These are of full resolution
        importance_map_full_res = inverse_spatial_transform(importance_map, z_where.view(B * space_cfg.G ** 2, 4),
                                                            (B * space_cfg.G ** 2, 1, *space_cfg.img_shape))

        # (B*G*G, 1, H, W) -> (B, G*G, 1, H, W)
        importance_map_full_res = importance_map_full_res.view(B, space_cfg.G ** 2, 1, *space_cfg.img_shape)
        ...
        y_each_cell = inverse_spatial_transform(y_att, z_where.view(B * space_cfg.G ** 2, 4),
                                                (B * space_cfg.G ** 2, 3, *space_cfg.img_shape)).view(B, space_cfg.G ** 2, 3,
                                                                                            *space_cfg.img_shape)
        ...
        # To full resolution
        # (B*G*G, 1, H, W) -> (B, G*G, 1, H, W)
        alpha_map_pure = inverse_spatial_transform(alpha_att_hat, z_where.view(B * space_cfg.G ** 2, 4),
                                                   (B * space_cfg.G ** 2, 1, *space_cfg.img_shape)).view(B, space_cfg.G ** 2, 1,
                                                                                               *space_cfg.img_shape)
        ...
        # (B*G*G, D) -> (B, G*G, D)
        z_what = z_what.view(B, space_cfg.G ** 2, space_cfg.z_what_dim)
        z_what_post = Normal(*[x.view(B, space_cfg.G ** 2, space_cfg.z_what_dim)
                               for x in [z_what_post.mean, z_what_post.stddev]])
        # (B, G*G, D)
        kl_z_what = kl_divergence(z_what_post, self.z_what_prior)

        # dimensionality check
        assert ((kl_z_pres.size() == (B, space_cfg.G ** 2, 1)) and
                (kl_z_depth.size() == (B, space_cfg.G ** 2, 1)) and
                (kl_z_scale.size() == (B, space_cfg.G ** 2, 2)) and
                (kl_z_shift.size() == (B, space_cfg.G ** 2, 2)) and
                (kl_z_what.size() == (B, space_cfg.G ** 2, space_cfg.z_what_dim))
                )
        ...
        # Dimensionality check
        assert (
                (z_pres.size() == (B, space_cfg.G ** 2, 1)) and
                (z_depth.size() == (B, space_cfg.G ** 2, 1)) and
                (z_scale.size() == (B, space_cfg.G ** 2, 2)) and
                (z_shift.size() == (B, space_cfg.G ** 2, 2)) and
                (z_where.size() == (B, space_cfg.G ** 2, 4)) and
                (z_what.size() == (B, space_cfg.G ** 2, space_cfg.z_what_dim))
        )
        log = {
            'fg': y_nobg,
            'z_what': z_what,
...
class ImgEncoderFg(nn.Module):
    ...
    def __init__(self):
        ...
        assert space_cfg.G in [4, 8, 16]
        # Adjust stride such that the output dimension of the volume matches (G, G, ...)
        last_stride = 2 if space_cfg.G in [8, 4] else 1
        second_to_last_stride = 2 if space_cfg.G in [4] else 1
        ...
        self.enc = nn.Sequential(
            ...
            nn.CELU(),
            nn.GroupNorm(32, 256),
            nn.Conv2d(256, space_cfg.img_enc_dim_fg, 1),
            nn.CELU(),
            nn.GroupNorm(16, space_cfg.img_enc_dim_fg)
        )
        ...
        self.enc_lat = nn.Sequential(
            nn.Conv2d(space_cfg.img_enc_dim_fg, space_cfg.img_enc_dim_fg, 3, 1, 1),
            nn.CELU(),
            nn.GroupNorm(16, space_cfg.img_enc_dim_fg),
            nn.Conv2d(space_cfg.img_enc_dim_fg, space_cfg.img_enc_dim_fg, 3, 1, 1),
            nn.CELU(),
        ...
        self.enc_cat = nn.Sequential(
            nn.Conv2d(space_cfg.img_enc_dim_fg * 2, 128, 3, 1, 1),
            nn.CELU(),
            nn.GroupNorm(16, 128)
        )

        # Image encoding -> latent distribution parameters (B, 128, G, G) -> (B, D, G, G)
        self.z_scale_net = nn.Conv2d(128, space_cfg.z_where_scale_dim * 2, 1)
        self.z_shift_net = nn.Conv2d(128, space_cfg.z_where_shift_dim * 2, 1)
        self.z_pres_net = nn.Conv2d(128 + moc_cfg.motion_input, space_cfg.z_pres_dim, 1)
        self.z_depth_net = nn.Conv2d(128, space_cfg.z_depth_dim * 2, 1)

        # (G, G). Grid center offset. (offset_x[i, j], offset_y[i, j]) is the center for cell (i, j)
        offset_y, offset_x = torch.meshgrid([torch.arange(space_cfg.G), torch.arange(space_cfg.G)])
    ...
    def forward(self, x, tau, global_step):
        ...
        # offset: (2, G, G) -> (G*G, 2)
        offset = self.offset.permute(1, 2, 0).view(space_cfg.G ** 2, 2)
        # (B, G*G, 2) and (G*G, 2)
        # where: (-1, 1)(local) -> add center points -> (0, 2) -> (-1, 1)
        z_shift = (2.0 / space_cfg.G) * (offset + 0.5 + z_shift.tanh()) - 1
        ...
        assert (
                ...
                (z_shift.size() == (B, space_cfg.G ** 2, 2)) and
                (z_scale.size() == (B, space_cfg.G ** 2, 2)) and
                (z_where.size() == (B, space_cfg.G ** 2, 4))
        )
...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc_what = nn.Linear(256, space_cfg.z_what_dim * 2)
...
class GlimpseDec(nn.Module):
    ...
    def __init__(self):
        ...
        self.dec = nn.Sequential(
            nn.Conv2d(space_cfg.z_what_dim, 256, 1),
            nn.CELU(),
            nn.GroupNorm(16, 256),
