...
class SpaceBg(nn.Module):
    ...
    def __init__(self):
        ...
        self.rnn_mask = nn.LSTMCell(space_cfg.z_mask_dim + space_cfg.img_enc_dim_bg, space_cfg.rnn_mask_hidden_dim)
        self.rnn_mask_h = nn.Parameter(torch.zeros(space_cfg.rnn_mask_hidden_dim))
        self.rnn_mask_c = nn.Parameter(torch.zeros(space_cfg.rnn_mask_hidden_dim))
        
        # Dummy z_mask for first step of rnn_mask
        self.z_mask_0 = nn.Parameter(torch.zeros(space_cfg.z_mask_dim))
        ...
        # ==== Prior related ====
        self.rnn_mask_prior = nn.LSTMCell(space_cfg.z_mask_dim, space_cfg.rnn_mask_prior_hidden_dim)
        # Initial h and c
        self.rnn_mask_h_prior = nn.Parameter(torch.zeros(space_cfg.rnn_mask_prior_hidden_dim))
        self.rnn_mask_c_prior = nn.Parameter(torch.zeros(space_cfg.rnn_mask_prior_hidden_dim))
        ...
        self.bg_sigma = space_cfg.bg_sigma
    ...
    def forward(self, x, global_step):
        ...
        z_mask = self.z_mask_0.expand(B, space_cfg.z_mask_dim)
        h = self.rnn_mask_h.expand(B, space_cfg.rnn_mask_hidden_dim)
        c = self.rnn_mask_c.expand(B, space_cfg.rnn_mask_hidden_dim)
        
        for i in range(space_cfg.K):
        ...
        h = self.rnn_mask_h_prior.expand(B, space_cfg.rnn_mask_prior_hidden_dim)
        c = self.rnn_mask_c_prior.expand(B, space_cfg.rnn_mask_prior_hidden_dim)
...
class ImageEncoderBg(nn.Module):
    ...
    def __init__(self):
        embed_size = space_cfg.img_shape[0] // 16
        nn.Module.__init__(self)
        self.enc = nn.Sequential(
            ...
            Flatten(),
            nn.Linear(64 * embed_size ** 2, space_cfg.img_enc_dim_bg),
            nn.ELU(),
        )
...
class PredictMask(nn.Module):
    ...
    def __init__(self):
        nn.Module.__init__(self)
        self.fc = nn.Linear(space_cfg.rnn_mask_hidden_dim, space_cfg.z_mask_dim * 2)
    ...
    def forward(self, h):
        ...
        """
        x = self.fc(h)
        z_mask_loc = x[:, :space_cfg.z_mask_dim]
        z_mask_scale = F.softplus(x[:, space_cfg.z_mask_dim:]) + 1e-4
...
class MaskDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.dec = nn.Sequential(
            nn.Conv2d(space_cfg.z_mask_dim, 256, 1),
            nn.CELU(),
            nn.GroupNorm(16, 256),
...
class CompEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc = nn.Sequential(
            ...
            nn.Linear(64 * embed_size ** 2, space_cfg.z_comp_dim * 2),
        )
    ...
    def forward(self, x):
        ...
        """
        x = self.enc(x[:, :4])
        z_comp_loc = x[:, :space_cfg.z_comp_dim]
        z_comp_scale = F.softplus(x[:, space_cfg.z_comp_dim:]) + 1e-4
...
class CompDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(space_cfg.z_comp_dim + 2, 32, 3, 1),
            nn.BatchNorm2d(32),
            nn.ELU(),
            nn.Conv2d(32, 32, 3, 1),
            nn.BatchNorm2d(32),
...
class CompDecoderStrong(nn.Module):
    ...
    def __init__(self):
        ...
        self.dec = nn.Sequential(
            nn.Conv2d(space_cfg.z_comp_dim, 256, 1),
            nn.CELU(),
            nn.GroupNorm(16, 256),
...
class PredictComp(nn.Module):
    ...
    def __init__(self):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(space_cfg.z_mask_dim, space_cfg.predict_comp_hidden_dim),
            nn.ELU(),
            nn.Linear(space_cfg.predict_comp_hidden_dim, space_cfg.predict_comp_hidden_dim),
            nn.ELU(),
            nn.Linear(space_cfg.predict_comp_hidden_dim, space_cfg.z_comp_dim * 2),
        )
