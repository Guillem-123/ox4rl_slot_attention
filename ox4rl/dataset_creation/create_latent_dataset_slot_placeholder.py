...
def create_latent_dataset(cfg, dataset_mode = "test", model=None):
    ...
    #TODO use trained model given as parameter or load based on config
    device = torch.device(cfg.device)
    model = SlotAttentionAutoEncoder(cfg.resolution, cfg.arch_slot.num_slots, cfg.arch_slot.hid_dim, output_channel=3, device=device.type).to(device)
